...
### 6.1 重载
方法重载是让类以统一的方式处理不同类型数据的一种手段。多个同名函数同时在，具有不同的参数个数/类型。返回值可以相同可以不同。

### 6.2 覆盖
覆盖又称为方法重写，是指子类重新实现父类继承过来的方法。
...
>>>> ![](https://github.com/HolyPaPa99/Core-Java-Concept/blob/master/images/adapter_pattern_uml_diagram.jpg)
